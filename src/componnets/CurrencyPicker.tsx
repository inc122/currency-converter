...
const CurrencyPicker = ({ value, onChanged }: IProps) => {
    ...
    const handleKeyDown = useCallback((e: KeyboardEvent<HTMLDialogElement>) => {
        ...
        switch (e.key) {
            case "ArrowDown":
                setFocusedIndex(v => Math.min(v + 1, filteredData.length - 1))
                break
            case "ArrowUp":
                setFocusedIndex(v => Math.max(v - 1, 0))
                break
            case "Enter":
                onChanged(filteredData[focusedIndex])
                break
        }
    }, [filteredData, focusedIndex, onChanged])
    ...
    useEffect(() => {
        const index = filteredData.findIndex(v => v.code === value.code)
    ...
    return (
        <div className="w-full">
            ...
            {showModal && (
                <dialog ref={dialogRef} className="rounded-sm p-[16px] w-full lg:w-[440px] h-[450px] lg:h-[439px] flex flex-col gap-[12px]" onClose={() => setShowModal(false)} onKeyDown={handleKeyDown}>
                        ...
                        {filteredData.map((d, i) => {
                            ...
                            return (
                                <button 
                                    ...
                                    id={d.code}
                                    key={d.name} 
                                    className={`flex items-center gap-[12px] px-[8px] py-[12px] w-full rounded-xs hover:bg-neutral-100 cursor-pointer ${focusedIndex === i ? 'bg-neutral-100' : ''}`}
